...
run.on("action", async (action) => {
  ...
  if (!run.session.actions.canExecute(action.id)) {
    throw new Error(`No wallet capability can execute ${action.request.kind}`);
  }
...
<Warning>
  Guest access on staging can read the Pipeline catalog but cannot execute Pipeline mutations. Use the OAuth scopes provisioned for your integration before testing stage, simulate, build, or commit against `chat-staging.aomi.dev`.
</Warning>
