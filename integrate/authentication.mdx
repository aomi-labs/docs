...
| --- | --- | --- |
| Node.js evaluation | Guest session with a cookie-aware fetch | [TypeScript quickstart](/integrate/typescript/quickstart) |
...
Guest mode is the default. In Node.js, pass the cookie-aware fetch from the quickstart:
...
<Warning>
  Plain Node.js `fetch` does not persist the cookie returned by Better Auth. Use the [tested quickstart wrapper](/integrate/typescript/quickstart) for evaluation, or OAuth for a real CLI or service. Cross-origin browser integrations should use the widget or provisioned browser OAuth flow.
</Warning>
