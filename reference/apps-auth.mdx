...
```
Client → POST /api/chat?app=mycoindex
           X-API-Key: sk-mcd-...
           X-Session-Id: session-uuid
              ↓
Backend → Lookup API key in database
              ↓
          Check key authorizes requested App
              ↓
          Authorized? → Load/create session → Route to App
          Denied?     → 401 (no key) or 403 (wrong scope)
```
