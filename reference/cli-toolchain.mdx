...
|---|---|---|
| `--email <EMAIL>` | — | Where ops send your activation code. **Required.** |
...
| Flag | Mirrors `aomi.toml` | Meaning |
|---|---|---|
| `[APP_RELEASE_TAG]` | — | Release to check. Falls back to deployment.json's `target.app_release_tag`. |
...
| Flag | Mirrors `aomi.toml` | Meaning |
|---|---|---|
| `[APP_RELEASE_TAG]` | — | Release to activate (e.g. `apps-my-bot-abc1234`). Falls back to deployment.json's `target.app_release_tag`. |
...
| `--path <DIR>` | — | Source repo for the deployment.json fallback. Default: `.` |
| `--dry-run` | — | Print the planned config request. No HTTP. |
| `--json` | — | Print the backend response as JSON. |
