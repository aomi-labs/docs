...
description: "Inspect Aomi's simulation-first pipeline, including Anvil fork lifecycle, state diffs, token changes, and safety guarantees."
---
...
Aomi uses a **simulation first** approach for all onchain transactions. Before any transaction reaches your wallet for signing, it is executed against a forked copy of the current network state. You review the exact outcome, including token changes, gas, and contract calls, before approving.
...
For the full host capability contract, see [Aomi App](/build/plugins/aomi-app).
...
| --- | --- | --- |
| Reverted | The contract reverted or the transaction would fail onchain | The agent surfaces the revert reason and does not commit |
...
- [Accounts and wallets](/concepts/accounts-and-wallets): wallet security model
- [Aomi App](/build/plugins/aomi-app): host tool contract for staging and committing
