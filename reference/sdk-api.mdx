...
<Warning>
Pin the platform requirement, not the crate's own version. The `aomi-sdk` crate ships at `3.0.3`, but the community platform requires `=3.0.3`. Match `platform.json`'s `required_sdk_version`, today `=3.0.3`, not the crate's latest.
</Warning>
