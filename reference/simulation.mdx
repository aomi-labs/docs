...
init_fork_providers(vec![params]).await?;
...
| --- | --- | --- |
| `SimulationReverted` | Contract reverted or transaction would fail on-chain | Fix parameters or check calldata and retry |
| `ForkCreationFailed` | RPC unavailable or rate limited | Retry with backoff |
| `InsufficientFunds` | Not enough ETH or tokens for the operation | Fund the account or notify user |
| `Unauthorized` | Sender lacks permission | Verify wallet/approval |
| `ForkNotInitialized` | `init_fork_providers` not called | Initialize before use |
