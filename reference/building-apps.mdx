...
<Warning>
  Pin the platform requirement, not the crate's own version. The `aomi-sdk` crate ships at `3.0.3`, but the community platform requires `=3.0.3`. Pin what `platform.json` names under `required_sdk_version`, today `=3.0.3`, not the crate's latest. A build pinned to anything else is rejected at activation.
</Warning>
