---
title: "Overview"
sidebarTitle: "Overview"
description: "Find detailed references for the Aomi SDK, CLI, HTTP API, hosted runtime, and authentication model."
---

Technical reference for everything you can call, configure, or extend.

## Build

<CardGroup cols={2}>
  <Card title="Rust SDK" icon="cube" href="/build/plugins/rust-sdk">
    Rust SDK for building agentic applications. ChatApp, builder pattern, custom tools.
  </Card>

  <Card title="Aomi App" icon="hammer" href="/build/plugins/aomi-app">
    Construct and deploy Aomi Apps with the Rust SDK.
  </Card>

  <Card title="Client CLI" icon="terminal" href="/reference/client-cli">
    Full reference for `@aomi-labs/client`.
  </Card>
</CardGroup>

## Integrate

<CardGroup cols={2}>
  <Card title="API Reference" icon="code" href="/reference/api-reference">
    HTTP endpoints, SSE streams, and request/response payloads.
  </Card>

  <Card title="Apps & Authentication" icon="key" href="/reference/apps-and-authentication">
    How API keys, Aomi Apps, and sessions work together.
  </Card>

  <Card title="Sessions" icon="clock-rotate-left" href="/reference/sessions">
    Chat session lifecycle, persistence, and threading.
  </Card>
</CardGroup>

## Runtime & Execution

<CardGroup cols={2}>
  <Card title="Runtime" icon="server" href="/reference/runtime">
    How the runtime executes an App turn. Model loop, transaction harness, streaming.
  </Card>

  <Card title="Simulation Reference" icon="flask" href="/reference/simulation-reference">
    Simulation first transaction pipeline.
  </Card>

  <Card title="Account abstraction" icon="user-shield" href="/concepts/account-abstraction">
    EIP-4337 and EIP-7702 execution, gas sponsorship, and batched operations.
  </Card>

  <Card title="Solana bundle" icon="sun" href="/concepts/solana-bundle">
    Atomic instruction bundles, simulation, signing, and submission routes.
  </Card>
</CardGroup>
