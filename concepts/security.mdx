---
title: Security
description: Aomi security architecture covering simulation-first transaction safety, non-custodial wallet model, API authentication, OWASP-compliant CI scanning, and the platform threat model.
---
...
flowchart TB
    ...
    subgraph "CI/CD Security"
        OWASP[OWASP Agentic Skills Scanners]
...
sequenceDiagram
    ...
    Edge->>Edge: Validate AOMI-APP-KEY format
...
| Cross-session data leak | Each session has its own history and tool state, with no shared memory |
...
<Info>
  This section covers the security properties of simulation. For the full technical deep-dive covering fork lifecycle, state diffing, and the tool API, see the [Simulation Reference](/reference/simulation).
</Info>
...
| **Per-transaction approval** | Every transaction requires explicit wallet confirmation, with no blanket approvals |
...
Skills published under `aomi-labs/skills` are security-audited against the [OWASP Agentic Skills Top 10](https://owasp.org/www-project-agentic-skills-top-10/) framework. AST03 (Over-Privileged Skills) is the category these checks target most directly. Every pull request runs four automated scanners in CI:
...
| **Malicious skill definitions** | 4 OWASP Agentic Skills Top 10 scanners run on every skill PR, blocked if any fail |
...
| **Wallet seed phrase custody** | Aomi cannot recover lost seed phrases, so users must back them up |
...
- [Execution Guide](/trade/execution): transaction lifecycle from simulation to settlement
