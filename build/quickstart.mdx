---
title: "Quickstart"
description: "Build and deploy your first Aomi App in about ten minutes, starting from an empty folder."
---
...
3. **Deploy.** The backend builds your plugin and creates a release. Use
   [`aomi-build`](/build/toolchain/aomi-build) from your terminal
   or the [Developer Platform](/build/developer-platform) from your browser.
...
<Frame caption="Describe the App you want to build or start from a focused template.">
  <img src="/images/build/create-app.jpg" alt="The Aomi Build creation screen with an App prompt and starter templates" />
</Frame>
...
<Steps>
  ...
  <Step title="Cargo.toml">
    Your App compiles to a `cdylib`. Pin `aomi-sdk` to the exact version the platform requires, today `=3.0.3`.
    ...
    <Warning>
    Pin the platform requirement `=3.0.3`, not the crate's own version. The `aomi-sdk` crate ships at `3.1.0`, but the community platform requires `=3.0.3`. Match the platform's `required_sdk_version` in `platform.json`, not whatever the crate prints. A build pinned to anything else is rejected at activation.
    </Warning>
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy and activate" href="/build/toolchain/aomi-build">
