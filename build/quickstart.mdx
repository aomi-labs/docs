...
<Steps>
  ...
  <Step title="Cargo.toml">
    ...
    <Warning>
    Pin the platform requirement `=3.0.3`, not the crate's own version. The `aomi-sdk` crate ships at `3.0.3`, but the community platform requires `=3.0.3`. Match the platform's `required_sdk_version` in `platform.json`, not whatever the crate prints. A build pinned to anything else is rejected at activation.
    </Warning>
  </Step>
