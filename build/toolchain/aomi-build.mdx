---
title: "aomi-build"
description: "The single CLI for the whole life of a plugin: scaffold, compile, connect, deploy, and activate. A verified end-to-end walkthrough first, then the full command and flag reference."
---

`aomi-build` is the single CLI for the whole life of a plugin: it scaffolds, compiles, deploys, and activates. A deployed plugin — its tools and system prompt, live on the runtime — is what Aomi calls an **App**, and that is the word you see in the CLI output, in `aomi.toml`, and on the Developer Platform.
...
<Note>
Prefer a browser? Everything on this page — connect, deploy, activate, status — also runs from the [Developer Platform](/build/platform) at `build.aomi.dev`, which additionally holds your secrets, model provider keys, bots, and usage. The CLI and the platform drive the same backend, so you can start in one and finish in the other.
</Note>
...
<Info>Reference verified against aomi-sdk@08b21f9 on 2026-06-29, updated against current source: `deploy status` and `deploy activate` are the canonical forms. Bare `aomi-build status` and `aomi-build activate` still work as aliases for one release cycle, then go away — prefer the `deploy`-prefixed forms.</Info>
...
<CardGroup cols={3}>
  ...
  <Card title="Plugin anatomy" icon="book" href="/build/plugins/anatomy">
    The full authoring walkthrough, from spec to curated tools to test.
  </Card>
  <Card title="Rust SDK" icon="cube" href="/build/plugins/sdk">
