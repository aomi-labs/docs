...
```bash
cargo install aomi-sdk --locked --features cli
```
...
```text
Required aomi-sdk: 5.1.1
Manifest: /path/to/my-app/Cargo.toml
Dependency: exact 5.1.1
Lockfile: matches 5.1.1
SDK check passed.
```
...
```text
  - Cargo.toml pins aomi-sdk 5.1.0, but this backend requires 5.1.1.
  - Cargo.lock resolves aomi-sdk 5.1.0, but this backend requires 5.1.1.
...
```text
    ...
    Updating aomi-sdk v5.1.0 -> v5.1.1
Required aomi-sdk: 5.1.1
Manifest: /path/to/my-app/Cargo.toml
Dependency: exact 5.1.1
Lockfile: matches 5.1.1
...
For an operator-owned credential, read the value at tool call time with
...
If every user supplies a separate credential, mark the declaration and use the
context-only resolver:

```rust
const USER_API_KEY: Secret =
    Secret::new("USER_API_KEY", "Your vendor API key.", true).user_owned();

let key = resolve_user_secret_value(
    &ctx,
    "USER_API_KEY",
    "Add USER_API_KEY in App settings before using this tool.",
)?;
```
...
<Info>Reference verified against published `aomi-sdk` 5.1.1 at commit `2ef3e04` on 2026-09-22. `deploy status` and `deploy activate` are the canonical forms.</Info>
