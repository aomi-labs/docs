---
title: "aomi-run"
description: "The local dev runtime. Load your compiled plugin, chat with it against a real LLM, and watch which tools the model picks — no backend, no deploy."
---

<Info>Verified against aomi-sdk@08b21f9 on 2026-06-29.</Info>

`aomi-run` is the local dev runtime. It loads a compiled plugin (the Rust `cdylib` you build with [aomi-build](/build/toolchain/aomi-build)), reads its manifest, and opens an interactive REPL wired to Anthropic, OpenAI, or OpenRouter. No backend, no deploy. You watch which tools the model picks and how they respond — it is how you feel out whether the model reaches for the right tools before you ever ship.
