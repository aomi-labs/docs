...
Aomi hosts the bot for you. You create a bot in Telegram, hand Aomi the token, and Aomi wires it up. You do not run a server. One bot can serve one App or several, so a single bot can front your whole catalog.
...
<Note>
Your App must appear in the **Attached apps** list, which means it has to be active first. If you do not see it, deploy or redeploy it (see [Deploy and activate on aomi CLI](/build/deploy-cli)), then reload the page. You can come back later and **edit the attached apps** for a bot without re-entering the token.
</Note>
