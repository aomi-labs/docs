...
<Steps>
  ...
  <Step title="Cargo.toml">
    Your App compiles to a `cdylib`. Pin `aomi-sdk` to the exact version the platform requires. Run `aomi-build sdk check` to get the current number, and pin it exactly with a leading `=`. The example below uses `=3.0.3`, the version the community platform requires today.
