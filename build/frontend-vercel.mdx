...
<Info>
**Verified 2026-09-22** against published `@aomi-labs/react` 0.7.6 and the public API origin `https://chat.aomi.dev`. Package versions move: trust the registry and your install over any version written here.
</Info>
...
```tsx
<AomiFrame backendUrl="https://chat.aomi.dev" height="100dvh" />
```
...
```tsx
<AomiFrame backendUrl="https://chat.aomi.dev" walletPosition={null} />
```
...
## Calling your App without the widget

Use the [headless library](/integrate/client-sdk) when you need your own UI or a
service integration. It calls the versioned `/v1/agent` resource and acquires
a bearer credential for that exact OAuth resource. The retired
`AOMI-APP-KEY` and `/api/thread/chat` contract is not part of the public
versioned API.
...
| The chat renders but answers never arrive | You are not signed in, or the backend URL is wrong. | Sign in from the control bar, and confirm `backendUrl` points at `https://chat.aomi.dev`. |
