...
<AccordionGroup>
  <Accordion title="SDK version mismatch: plugin=X, host=Y">
    ...
    <Warning>
    Pin the platform requirement, not the crate's own version. The `aomi-sdk` crate ships at `3.1.0`, but the community platform requires `=3.0.3`. For a ship build, match `platform.json`'s `required_sdk_version` exactly: `aomi-sdk = "=3.0.3"`.
    </Warning>
  </Accordion>
