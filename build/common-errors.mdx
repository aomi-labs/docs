...
<AccordionGroup>
  <Accordion title="SDK version mismatch: plugin=X, host=Y">
    ...
    <Warning>
    For a ship build, pin exactly to the platform's required version. Run `aomi-build sdk check` to read it, or `aomi-build sdk fix` to set it, then pin with a leading `=`, for example `aomi-sdk = "=3.1.0"`. Do not hardcode a number from this page; it moves.
    </Warning>
  </Accordion>
