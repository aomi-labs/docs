...
For the full plugin lifecycle (crate setup, building, and shipping), see [Plugin anatomy](/build/plugins/anatomy). This page focuses on the tool surface.
...
The arms are `app`, `name`, `version`, `preamble`, `tools`, optional `secrets`, and optional `namespaces`. `namespaces` defaults to `["evm-core"]` when omitted, which gives your tools the host's EVM wallet flows. Add `secrets = [...]` to declare per-plugin credentials the host gates App load on.
...
- [Plugin anatomy](/build/plugins/anatomy): full plugin lifecycle and host tool contract
- [Evals and testing](/build/plugins/evals): test tool and App behavior
- [Rust SDK](/build/plugins/sdk): full Rust SDK documentation
