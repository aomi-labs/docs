---
title: Rust SDK
description: Rust reference for the Aomi plugin SDK. Build a plugin as a cdylib with typed tools, the DynAomiTool trait, the dyn_aomi_app! macro, secrets, host namespaces, and the runtime hot load model.
---

<Info>Verified against aomi-sdk@08b21f9 on 2026-06-29.</Info>

The Aomi SDK is a Rust crate for building **plugins**. A plugin is a shared library (`cdylib`) that the Aomi runtime hot loads over a stable C ABI. You write typed tools in Rust, compile the plugin to a `.so` (Linux) or `.dylib` (macOS), and the runtime loads it without a restart. Deployed on the runtime, a plugin is what Aomi calls an App.
...
<CardGroup cols={2}>
<Card title="Plugin anatomy" icon="hammer" href="/build/plugins/anatomy">
