---
title: "Aomi App"
description: "Learn how an Aomi App is organized: the plugin crate, manifest, preamble, tools, and hosted agentic loop."
---

<Info>Verified against aomi-sdk@08b21f9 on 2026-06-29.</Info>

Aomi uses a plugin model similar to ChatGPT plugins and Claude connectors. You author a small Rust plugin that wraps your APIs as typed tools and defines an agentic loop with a preamble.

We call the deployed result an **Aomi App** because the agentic loop belongs to your product. You embed its role, tools, and workflow in the software you ship. Aomi hosts and runs that loop, connects it to users, and gives it a controlled path to compose, simulate, sign, and broadcast blockchain actions.

In these docs, a **plugin** is the Rust crate you write. An **App** is the deployed, user-selectable unit. The CLI, `aomi.toml`, and Developer Platform use **App**. This page explains how to author the plugin that powers an Aomi App: how the crate is laid out, what goes in each file, and how you wire it together.
...
<Note>
  The earlier docs talked about `aomi_chat::CoreAppBuilder`. That is an internal runtime crate and is not how you author a plugin. The real authoring model is the one below: the `dyn_aomi_app!` macro plus the `DynAomiTool` trait from the public `aomi-sdk` crate.
</Note>

## What you are building

The Aomi SDK lets you wrap any external API as a plugin. Once deployed, that plugin powers an Aomi App. Aomi loads it and routes conversations to its agentic loop. The plugins in the SDK repo show the range you can cover:
...
Every plugin follows the same layout across three files. Keep this split even for small plugins. It keeps the surface the model sees easy to scan and keeps API details out of your tool logic.
...
`lib.rs` is short on purpose. It declares the other modules, holds the preamble, and calls `dyn_aomi_app!` to register the plugin. Here is the full `lib.rs` from the HTTP template:
...
| `namespaces` | The host capability sets your plugin depends on. `[]` for a read only HTTP wrapper (see "Declaring host namespaces"). |
...
<Note>
  For full trait signatures (`DynAomiTool`, `DynToolCallCtx`, the `dyn_aomi_app!` macro, and the `aomi_sdk::testing` helpers like `TestCtxBuilder` and `run_tool`), see the [Rust SDK](/build/plugins/rust-sdk).
</Note>

## Declaring host namespaces

The `namespaces` field in `dyn_aomi_app!` declares which host capabilities your plugin needs. A read only HTTP wrapper needs none, so it uses `namespaces = []`, like the template above. A plugin that reads chain state or stages transactions declares a namespace, the way the `defillama` plugin does:
...
Host capabilities are a public contract, not private infrastructure. Plugins that execute transactions may assume the host runtime exposes tools such as:
...
<Note>
  Plugins that execute across several steps (like `polymarket` or `khalani`) return a `ToolReturn` with route hints instead of a bare JSON value, so the host can chain a wallet signature back into the next tool call. The full route hint contract lives in the SDK repo's `docs/host-interop.md` and the [Rust SDK](/build/plugins/rust-sdk).
</Note>

## The aomi.toml manifest

If you are building a plugin for a **community or partner platform** in your own source repo, you also author an `aomi.toml`. This manifest tells the deploy tooling where your App ships and how the backend should load it. (Official plugins that live in the SDK repo do not use `aomi.toml`; they ship through the repo's release pipeline instead.)
...
[dependencies]
aomi-sdk   = "=3.0.3"          # match platform.json's required_sdk_version
...
<Warning>
  Pin the platform requirement, not the crate's own version. The `aomi-sdk` crate ships at `3.1.0`, but the community platform requires `=3.0.3`. Pin what `platform.json` names under `required_sdk_version`, today `=3.0.3`, not the crate's latest. A build pinned to anything else is rejected at activation.
</Warning>

## Authoring guidelines

- Prefer one plugin crate per external product or ecosystem.
- Keep the toolset small. 3 to 8 tools per plugin is typical for a clean workflow.
...
- If your plugin needs signing or execution, depend only on the public host capabilities above.
...
<CardGroup cols={2}>
  <Card title="Compile, deploy, and activate" icon="hammer" href="/build/toolchain/aomi-build">
    Use the `aomi-build` CLI to compile your plugin and `aomi-run` to exercise it locally, then `aomi-build deploy` and `aomi-build deploy activate` to ship it.
  </Card>
