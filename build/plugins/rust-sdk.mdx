---
title: Rust SDK
description: "Use the Rust SDK to build loadable Aomi plugins with typed tools, secrets, host namespaces, and the dyn_aomi_app! registration macro."
---

<Info>Verified against aomi-sdk@08b21f9 on 2026-06-29.</Info>

The [Aomi SDK](https://github.com/aomi-labs/aomi-sdk/tree/main/sdk) is a Rust crate for building **plugins**, published as [`aomi-sdk` on crates.io](https://crates.io/crates/aomi-sdk). A plugin is a shared library (`cdylib`) that the Aomi runtime hot loads over a stable C ABI. You write typed tools in Rust, compile the plugin to a `.so` (Linux) or `.dylib` (macOS), and the runtime loads it without a restart. Deployed on the runtime, a plugin is what Aomi calls an App.
...
A minimal plugin needs four things:
...
<Note>
Pin `aomi-sdk` exactly (`=3.0.3`), not with a caret range. The runtime gates plugin loading on an exact SDK version match, so the pin must equal the platform's `required_sdk_version` (see `platform.json` in the platform repo). A mismatch fails before your plugin ever loads.
</Note>

<Warning>
Pin the platform requirement, not the crate's own version. The `aomi-sdk` crate ships at `3.1.0`, but the community platform requires `=3.0.3`. Match `platform.json`'s `required_sdk_version`, today `=3.0.3`, not the crate's latest.
</Warning>
...
| Member | What it is |
| --- | --- |
| `type App` | The app struct this tool belongs to. Ties the tool to one plugin. |
...
<Tip>
Prefer intent shaped names like `search_*`, `get_*`, `build_*`, and `submit_*` over raw endpoint wraps. Keep the set small. Three to eight tools per plugin is typical for a clean workflow.
</Tip>
...
`dyn_aomi_app!` is the one macro you call per plugin. It wires your tools into a dispatch router, builds the plugin manifest the host reads, and generates the FFI exports the runtime calls across the C ABI. You do not implement `DynAomiApp` or any C function by hand.
...
| `version` | Semver string for this plugin, such as `"0.1.0"`. |
| `preamble` | The system prompt injected into the LLM context for this App. |
| `tools` | The list of tool structs to register. |
| `secrets` | Optional. The named credentials this plugin declares. See below. |
| `namespaces` | Host namespaces this plugin needs. Defaults to `["evm-core"]`. |

## Secrets

If your plugin needs external credentials, declare them as `Secret` slots. Each slot has a canonical name in `SCREAMING_SNAKE_CASE`, a one sentence description shown in the settings UI, and a `required` flag.
...
The `namespaces` field lists host side capability sets the runtime injects alongside your own tools. The default is `["evm-core"]`, which most plugins want for EVM wallet flows.

| You want to… | Set namespaces to |
| --- | --- |
| Use the default EVM wallet flow | omit the field, or `["evm-core"]` |
| Ship a namespace only plugin (no wallet flow) | `["database"]` |
...
A minimal greeter plugin, straight from the crate docs:
...
The reference template at `sdk/examples/app-template-http` in the `aomi-labs/aomi-sdk` repo shows the recommended file split for a real plugin: `src/lib.rs` for the manifest and preamble, `src/client.rs` for the HTTP client and typed args, and `src/tool.rs` for the tool implementations.

## Next steps

<CardGroup cols={2}>
<Card title="Aomi App" icon="hammer" href="/build/plugins/aomi-app">
The full workflow for authoring, structuring, and publishing a plugin.
</Card>
