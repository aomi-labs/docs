...
- **The portal.** A no-code path in your browser. Click Dry run, Deploy, Activate. This is the recommended path for your first deploy.
...
The portal is the no-code path and the recommended way to ship your first App. These steps are verified working as of 2026-06-29: Dry run, Deploy, and CI all succeed.

<Steps>
  <Step title="Open the settings page">
    Go to `https://chat.aomi.dev/settings`.
  </Step>
  <Step title="Sign in with GitHub">
    Click "Sign in with GitHub". Authorize the Aomi Build GitHub App and grant it access to the repository that holds your App.
  </Step>
  <Step title="Find your repository">
    After sign in, your repository appears under "Source Repositories" with an installation id and three actions: Dry run, Deploy, and Activate.
  </Step>
  <Step title="Dry run">
    Click "Dry run". This previews the deployment manifest and runs the preflight checks. It changes nothing. Confirm the checks pass.
  </Step>
  <Step title="Deploy">
    Click "Deploy". The backend builds your App as a cdylib, CI runs, and a release is published. When it finishes you see "Build is ready for activation".
  </Step>
  <Step title="Activate">
    Click "Activate" to load your App into the live runtime. This is the final step that makes your App live.
  </Step>
</Steps>

<Note>
Activation is the final gate. Your App must pass the platform's checks first, including the SDK version pin. Once it passes and you activate, your App is live in the runtime.
</Note>
