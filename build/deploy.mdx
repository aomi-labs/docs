---
title: Deploy and activate on Dashboard
description: Ship an Aomi App from your browser with the no-code deploy dashboard. Preflight, Deploy, Activate, plus what every deploy field means and how the validation pipeline works.
---

The quickstart gets you to a deployed App on the happy path. This page is the rest of the story: the no-code dashboard path, what every deploy field means, what the validation pipeline checks, and how activation actually works.
...
Prefer the terminal? The CLI runs the same flow as the dashboard: `connect` once, then `deploy`, then `activate`, all from your source repo. It has its own page with the full walkthrough and the real output at every step.

<Card title="Deploy and activate on aomi CLI" href="/build/deploy-cli" icon="terminal">
The scriptable path: install the CLI, connect your repo, and deploy with `aomi-build`.
</Card>
