...
```text
Next steps:
  Watch CI: https://github.com/aomi-labs/community-apps/actions
  Once green, request activation:  aomi-git activate --request
```
...
Here is what each stage checks in plain English, and what a failure means.
...
This reads your `.aomi/deployment.json` and posts an **Activation requested** message into the `✅-activation-requests` Discord channel, pinging the **@ops** role. The message carries your App name, the repo, the **release tag**, and your target tags. Every request lands in one place instead of scattered DMs.
