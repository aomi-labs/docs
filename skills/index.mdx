---
title: "Aomi Skills"
description: "Install Agent Skills that teach coding assistants to use Aomi. Use aomi-transact for trading and aomi-build for App development."
---
...
This is the skill behind [Build on Aomi](/build/overview).
...
You need `@aomi-labs/client` v0.4.4 or newer. For transaction signing, also install `viem`:

```bash
npm install -g viem
```

`aomi-build` targets the Rust `aomi-sdk` (v0.1.15 or newer) when you compile the App you scaffold. See [Build on Aomi](/build/overview) for that toolchain.

## Where they live

Both skills are open source in the [`aomi-labs/skills`](https://github.com/aomi-labs/skills) repo, under the MIT license. Every change runs four security scanners in CI. For the full list of protocols `aomi-transact` can reach, see the [protocol catalog](/guides/skills).
...
<CardGroup cols={2}>
  ...
  <Card title="Build on Aomi" icon="hammer" href="/build/overview">
