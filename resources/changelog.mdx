---
title: "Changelog"
description: "Follow Aomi SDK, widget, runtime, and integration updates, with a record of major product changes and current priorities."
---
...
- **Deploy from the portal.** A no-code walkthrough at [Deploy and activate](/build/toolchain/aomi-build): sign in at the settings page, then Dry run, Deploy, and Activate.
...
- **Account abstraction support.** EIP-4337 and EIP-7702 execution can batch calls into one operation and request network-gas sponsorship. See [Account abstraction](/concepts/account-abstraction).
...
- **`@aomi-labs/client` CLI GA.** Full reference at [CLI Reference](/reference/client-cli); validate prompts, inspect sessions, and simulate execution locally.
...
- **Simulation pipeline with Anvil fork management.** Every transaction simulated on a forked network before reaching the wallet. See [Simulation Reference](/reference/simulation-reference).
...
- Sessions are now keyed by `X-Session-Id` UUID with separate `AOMI-APP-KEY` for App scoping. See [Apps & Authentication](/reference/apps-and-authentication).
...
- [What is Aomi?](/build/overview): platform overview
