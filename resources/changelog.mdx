...
- **`@aomi-labs/react` headless library** — `AomiRuntimeProvider`, `useAomiRuntime`, `useControl`, `useUser`, and the `BackendApi` client for building custom UIs.
...
- Sessions are now keyed by `X-Session-Id` UUID with separate `X-API-Key` for App scoping. See [Apps & Authentication](/reference/apps-auth).
