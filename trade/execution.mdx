...
`--aa` and `--eoa` are mutually exclusive. You cannot combine `--aa-provider` or `--aa-mode` with `--eoa`. See [Transactions and signing](/reference/client-cli/transactions-and-signing#signing) for the full signing surface.
