...
<Info>
  ...
  wallet or provider proof for an origin-bound access token. `AomiWidget`
  receives no paymaster URL, gas policy, fee rate, treasury, or executor
  address; Aomi resolves those settings from the Application ID.
</Info>
