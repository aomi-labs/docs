...
Tests that only care about the payload can read `output.value`. Tests that exercise multi-step routing can assert on `output.routes`.
...
| Assert on `routes` for multi-step tools | Confirm the tool hands off the right follow-up step |
