...
- An **`App`** — the app struct the tool belongs to.
- An **`Args`** struct — the typed parameters the LLM generates. It must derive `Deserialize` and `JsonSchema`.
- A **`NAME`** — the identifier the LLM uses to invoke the tool.
- A **`DESCRIPTION`** — tells the LLM when to use the tool.
- A **`run`** function — the logic. It returns a JSON `Value` on success or an error `String`.
